    ...
    try {
        ...
        if (!document.getElementById('mindsync-exact-styles')) {
            ...
            style.textContent = `
                ...
                .ms-option-item.selected { border: 2.5px solid #1B2559; box-shadow: 0 5px 15px rgba(0,0,0,0.1); }
...
async function renderIntro(container) {
    ...
    container.innerHTML = `
        <div class="ms-page animate-fade">
...
function renderQuestion(container) {
        ...
        <div class="ms-page animate-fade">
            <div class="ms-wrapper">
                ...
                <div class="ms-progress-card">
                    <div class="ms-progress-text">Savollar: ${currentQ + 1} / ${test.questions.length}</div>
                    <div class="ms-progress-bg"><div class="ms-progress-fill" style="width: ${progress}%"></div></div>
                </div>
            ...
            q.options.map((o, i) => `
                ...
                <div class="ms-timer-area">
                    <div class="ms-circular">
                        <svg width="100" height="100">
                            ...
                            <circle class="prog" id="timer-prog" cx="50" cy="50" r="45" style="stroke-dasharray: 283; stroke-dashoffset: ${283 - (totalTimeSeconds / ((test.timeLimit || 30) * 60)) * 283}"></circle>
                        </svg>
                ...
                <div class="ms-nav-area">
                    <button class="ms-prev-btn" id="go-prev" ${currentQ === 0 ? 'style="opacity:0.3; cursor:default"' : ''}><i data-lucide="arrow-left" style="width:20px; color:white"></i></button>
                    <button class="ms-next-btn" id="go-next">Keyingi savol <i data-lucide="arrow-right" style="width:18px"></i></button>
                </div>

                <div class="ms-dots">
                    ${test.questions.map((_, i) => `<div class="ms-dot ${i === currentQ ? 'active' : ''}"></div>`).join('')}
                </div>
            </div>
        </div>
    `;
    ...
    const doNext = () => {
        ...
        else { currentQ++; renderQuestion(container); }
    };
